...
if command -v uv &>/dev/null; then
  uv tool install --editable . 2>/dev/null && \
    echo '{"continue": true, "suppressOutput": true, "status": "installed via uv tool"}' && exit 0
fi

if command -v pipx &>/dev/null; then
  pipx install --editable . 2>/dev/null && \
    echo '{"continue": true, "suppressOutput": true, "status": "installed via pipx"}' && exit 0
fi

pip install -e . --quiet 2>/dev/null && \
  echo '{"continue": true, "suppressOutput": true, "status": "installed via pip"}' && exit 0
